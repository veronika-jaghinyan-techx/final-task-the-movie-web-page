...
function storeInitialState() {
    ...
    Array.from(getTrackableControls()).forEach(control => {
        const key = control.id || control.getAttribute('name') || (control.classList.contains('k-draghandle') ? `handle-${Math.random()}` : control.classList[0]);
        if (key)
...
function checkForChanges() {
    let hasChanged = Array.from(getTrackableControls()).some(control => {
        const key = control.id || control.getAttribute('name') || (control.classList.contains('k-draghandle') ? `handle-${control.getAttribute('data-handle-id')}` : control.classList[0]);
        const initialValue = initialValues.get(key);
        const currentValue = getCurrentControlValue(control);
        return initialValue !== undefined && String(currentValue) !== String(initialValue);
    });
    if (!hasChanged)
...
function initializeMutationObservers() {
    ...
    const observerCallback = () => { checkForChanges(); };
    const config = { subtree: true, attributes: true, attributeFilter: ['aria-valuenow', 'style'] };
    if (sortButton)
        new MutationObserver(observerCallback).observe(sortButton, { attributes: true, childList: true, characterData: true });
    if (languageButton)
        new MutationObserver(observerCallback).observe(languageButton, { attributes: true, childList: true, characterData: true });
    if (genreContainer)
        new MutationObserver(observerCallback).observe(genreContainer, { subtree: true, attributes: true, childList: true, characterData: true });
...
function toggleLargeSearchButton() {
    ...
    mutationObserver.observe(searchButton, { attributes: true, attributeFilter: ['disabled'] });
