...
export function initHeader() {
    ...
    searchButtons.forEach((button) => {
        ...
        button.addEventListener("click", (e) => {
            ...
            const nav = button.closest("[data-header-nav]");
            if (!isHTMLElement(nav))
                return;
            const searchWrapper = nav.querySelector(".header-search-wrapper");
            if (!isHTMLElement(searchWrapper))
                return;
    ...
    const searchResults = document.querySelectorAll(".header-search-result-section");
    const searchForms = document.querySelectorAll(".search-form");
    searchForms.forEach((searchForm) => searchForm.addEventListener("submit", (e) => e.preventDefault()));
    searchInputs.forEach((searchInput) => {
        searchInput.addEventListener("input", () => {
            if (searchInput.value.trim() !== "") {
                searchResults.forEach((searchResult) => searchResult.classList.remove("hide"));
            }
            else {
                searchResults.forEach((searchResult) => searchResult.classList.add("hide"));
            }
        });
    });
    document.addEventListener("click", (e) => {
    ...
    const burgerToggle = document.querySelector("#burgerMenuButton");
    const headerMenuMobile = document.querySelector("#headerMenuMobile");
    if (!isHTMLElement(burgerToggle) || !isHTMLElement(headerMenuMobile))
        return;
    ...
    document.addEventListener("click", (e) => {
        const isHidden = headerMenuMobile.classList.contains("hide");
        if (!headerMenuMobile.contains(e.target) &&
    ...
    const mobileMenuButtons = document.querySelectorAll("[data-menu-button]");
