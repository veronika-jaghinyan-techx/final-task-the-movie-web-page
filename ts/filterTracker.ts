const initialValues: Map<string, string | boolean> = new Map();
...
function getTrackableControls(): NodeListOf<Element> {
	return filterSection.querySelectorAll(
		'input[type="checkbox"], input[type="date"], #keywordSearch, ' +
...
function getCurrentControlValue(element: Element): string | boolean {
...
function storeInitialState(): void {
	...
	Array.from(getTrackableControls()).forEach((control) => {
		const key =
			...
			(control.classList.contains("k-draghandle")
				? `handle-${Math.random()}`
				: control.classList[0]);
		if (key) initialValues.set(key, getCurrentControlValue(control));
	});
}

function checkForGenreChanges(): boolean {
...
function checkForChanges(): void {
	...
	if (!hasChanged) hasChanged = checkForGenreChanges();
	if (searchButton) searchButton.disabled = !hasChanged;
}

function initializeMutationObservers(): void {
	...
	sliderHandles.forEach((handle, index) =>
		handle.setAttribute("data-handle-id", String(index))
	);
	...
	sliderHandles.forEach((handle) =>
		new MutationObserver(observerCallback).observe(handle, config)
	);
}

export function initializeFilterTracker(): void {
	if (!searchButton || !filterSection) return;
	storeInitialState();
	...
	document.addEventListener("mousedown", (event) => {
		if ((event.target as HTMLElement).classList.contains("k-draghandle"))
			checkForChanges();
	});
...
function toggleLargeSearchButton(): void {
	const searchButton = document.getElementById(
		"searchButton"
	) as HTMLButtonElement;
	const searchButtonLarge = document.getElementById(
		"searchButtonLarge"
	) as HTMLButtonElement;

	if (!searchButton || !searchButtonLarge) return;
