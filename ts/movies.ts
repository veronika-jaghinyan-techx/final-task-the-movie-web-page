...
import { Genre, GenreResponse, Movie, MovieResponse, Range } from "./types.js";
...
export function initMovies() {
	...
	async function getMovies(page: number): Promise<void> {
		...
		try {
			...
			data.results.forEach((movie) => {
				...
				movieCardImgLink.href = getSingleMovieUrl(
					movie.id,
				...
				ctx.fillText(
					"%",
					center + ctx.measureText(String(percent)).width + 3,
					center - 4
				);
	 ...
	 */
	function getQueryParams(page: number): string {
		// 1. Genres
		...
		const selectedSort = document.querySelector("#sort-by") as HTMLElement;
		...
		const allReleases = document.querySelector(
			"#all_releases"
		) as HTMLInputElement;
		const getAllCountries = document.querySelector(
			"#all_release_countries"
		) as HTMLInputElement;

		// 3. Keywords
		const selectedKeywordIds: (string | undefined)[] = Array.from(
			document.querySelectorAll<HTMLSpanElement>("#badgeArea > span"),
			(span) => span.dataset.id
		);

		// 4. Release Type
		const releaseTypes = Array.from(
			document.querySelectorAll('input[name="release_type"]:checked')
		...
		const obj: QueryParams = {
			...
			"vote_count.gte": sliderValues.vote_count.gte as number,
			"vote_average.gte": sliderValues.vote_average.gte as number,
			"vote_average.lte": sliderValues.vote_average.lte as number,
			"with_runtime.gte": sliderValues.with_runtime.gte as number,
			"with_runtime.lte": sliderValues.with_runtime.lte as number,
	...
	if (isHTMLButtonElement(searchButton)) {
		searchButton.addEventListener("click", () => {
			...
			moviesState.endpoint = (page: number) => {
	...
	if (isHTMLButtonElement(searchButtonLarge)) {
		searchButtonLarge.addEventListener("click", () => {
			if (isHTMLButtonElement(searchButton)) searchButton.click();
		});
	}
	...
	async function getGenres(): Promise<void> {
		...
		try {
			...
			if (!isHTMLElement(genresContainer)) return;
	 ...
	 */
	const sortFilterHeaders = document.querySelectorAll<HTMLButtonElement>(
		"[data-sort-filter-header]"
	);
