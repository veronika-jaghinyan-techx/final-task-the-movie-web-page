 ...
 */
export function initTooltip(selector: string = "#globalTooltip") {
	const tooltip = document.querySelector<HTMLElement>(selector);

	if (!isHTMLElement(tooltip)) return;

	function showTooltip(e: MouseEvent): void {
		const target = e.target;

		if (!(target instanceof HTMLElement) || !tooltip) return;
